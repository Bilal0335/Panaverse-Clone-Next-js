...
const ProgrammDataTwo: React.FC<ProgrammDataTwoProps> = ({ selectItems, setSelectedItems }) => { 
  const programmData = [
    {
      slug: "wmd",
  ...
  const selectedItemData = programmData.find(item => item.slug === selectItems);
      ...
      {programmData.map(item => (
        ...
        >
          ...
          <div>
            ...
            <h3 className="text-xl font-semibold leading-tight">{item.header}</h3>
          </div>
        </div>
      ))}
    </div>
